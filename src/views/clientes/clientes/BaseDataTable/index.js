...
injectReducer('dataClientes', reducer);
...
const BaseDataTable = forwardRef((props, ref) => {
	...
	const data = useSelector((state) => state.dataClientes.data.data);
	const loading = useSelector((state) => state.dataClientes.data.loading);
	const { current_page, perPage, total, search, sort } = useSelector((state) => state.dataClientes.data.tableData);
